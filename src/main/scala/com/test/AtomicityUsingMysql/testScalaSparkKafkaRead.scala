...
object testScalaSparkKafkaRead extends SparkOpener{
  ...
  def main(args:Array[String]):Unit={
    ...
    val df = spark.readStream.format(sourceKafka).option("kafka.bootstrap.servers", bootstrapServers).option("value.deserializer",valueSerializer).option("key.deserializer",keySerializer).option("startingOffsets", "earliest").option("subscribe", topicName).load.selectExpr("*","split(value,'~') as valueSplitted").drop("value").selectExpr("offset", "topic", "timestamp", "cast(valueSplitted[0] as string)as value", "cast(valueSplitted[1] as string) as date", "timestampType", "partition", "key")
